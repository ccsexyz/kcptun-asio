...
class snappy_stream_reader final : public std::enable_shared_from_this<snappy_stream_reader> {
public:
...
private:
    ...
    char chunk_[snappy_max_block_size+snappy_checksum_size+snappy_header_len];
    char decode_buffer_[snappy_max_block_size];
};

class snappy_stream_writer final : public std::enable_shared_from_this<snappy_stream_writer> {
public:
...
private:
    asio::io_service &service_;
    OutputHandler out_handler_;
    char buf_[snappy_max_block_size+snappy_checksum_size+snappy_header_len+snappy_magic_head_len];
    std::size_t off_;
    Task task_;
};
