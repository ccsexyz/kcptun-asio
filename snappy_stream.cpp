...
static const unsigned char magic_head[] = {0xff, 0x06, 0x00, 0x00, 0x73, 0x4e, 0x61, 0x50, 0x70, 0x59};

snappy_stream_writer::snappy_stream_writer(asio::io_service &io_service, OutputHandler handler)
        : service_(io_service), out_handler_(handler) {
    memcpy(buf_, magic_head, snappy_magic_head_len);
    off_ = snappy_magic_head_len;
}

void snappy_stream_writer::async_input(char *buf, std::size_t len, Handler handler) {
    ...
    out_handler_(buf_, off_, [this, self, handler](std::error_code ec, std::size_t) {
        off_ = 0;
        if (handler) {
            handler(ec, 0);
        }
...
void snappy_stream_reader::async_input(char *buf, std::size_t len, Handler handler) {
    auto self = shared_from_this();
    ...
    uint32_t chunk_len =
            uint32_t(uint8_t(chunk_[1])) | (uint32_t(uint8_t(chunk_[2])) << 8) | (uint32_t(uint8_t(chunk_[3])) << 16);
    if (!valid_) {
        if (chunk_type == chunk_type_stream_identifier && chunk_len == snappy_magic_head_len - snappy_header_len &&
            off_ < snappy_magic_head_len) {
    ...
    if (chunk_type == chunk_type_uncompressed_data) {
        uint32_t crc32_chksum = crc32c_cast(0, (unsigned char *) (chunk_ + snappy_header_len + snappy_checksum_size),
                chunk_len - 4);
        ...
        out_handler_(chunk_ + snappy_header_len + snappy_checksum_size,
        ...
        std::size_t compressed_length = off_ - (snappy_header_len + snappy_checksum_size);
        std::size_t uncompressed_length;
        snappy::GetUncompressedLength(compressed, compressed_length, &uncompressed_length);
        snappy::RawUncompress(compressed, compressed_length, decode_buffer_);
        uint32_t crc32_chksum = crc32c_cast(0, (unsigned char *) (decode_buffer_), uncompressed_length);
        if (crc32_chksum != crc32_chksum_2) {
            if (handler) {
                handler(std::error_code(1, std::generic_category()), len);
            }
        ...
        out_handler_(decode_buffer_, uncompressed_length,
                [this, self, buf, len, handler](std::error_code ec, std::size_t) {
                    off_ = 0;
                    if (ec) {
                        if (handler) {
