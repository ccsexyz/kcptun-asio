...
void kcptun_client::run() {
    if (!global_config.nocomp) {
        ...
        snappy_writer_ = std::make_shared<snappy_stream_writer>(
            service_, [this](char *buf, std::size_t len, Handler handler) {
...
void kcptun_client::do_receive() {
    ...
    local_->async_read_some(
        buf_, sizeof(buf_), [this, self](std::error_code ec, std::size_t len) {
            ...
            if (snappy_reader_) {
                snappy_reader_->async_input(
                    buf_, len, [this, self](std::error_code ec, std::size_t) {
                        if (ec) {
                            return;
                        }
                        do_receive();
...
void kcptun_client::snappy_stream_writer_output_handler(char *buf,
                                                        std::size_t len,
                                                        Handler handler) {
    auto self = shared_from_this();
    local_->async_write(
        buf, len, [this, self, handler, len](std::error_code ec, std::size_t) {
            if (handler) {
                handler(ec, len);
            }
        });
...
void kcptun_client::snappy_stream_reader_output_handler(char *buf,
                                                        std::size_t len,
                                                        Handler handler) {
    auto self = shared_from_this();
    smux_->input(buf, len,
                 [this, self, handler, len](std::error_code ec, std::size_t) {
                     if (handler) {
                         handler(ec, len);
                     }
                 });
